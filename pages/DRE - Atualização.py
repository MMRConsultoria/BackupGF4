...
with tab_audit:
    ...
    # Listar planilhas e armazenar em session_state
    if "planilhas_audit" not in st.session_state or st.session_state.get("last_subpastas") != s_ids_audit:
        planilhas = list_spreadsheets_in_folders(drive_service, s_ids_audit)
        st.session_state.planilhas_audit = planilhas
        st.session_state.last_subpastas = s_ids_audit
        # Inicializa resultados vazios
        st.session_state.resultados_audit = {p["id"]: None for p in planilhas}

    # Mostrar tabela com checkboxes para seleção
    planilhas = st.session_state.planilhas_audit
    planilhas_names = [p["name"] for p in planilhas]
    selecionadas = st.multiselect("Selecione planilhas para auditar:", options=planilhas_names, key="planilhas_selecionadas")
    ...
    if st.button("📊 EXECUTAR AUDITORIA", use_container_width=True):
        ...
        # Carregar origem FATURAMENTO uma vez (igual antes)
        # ... seu código para carregar df_o_fat_p ...

        # Rodar auditoria só nas planilhas selecionadas
        for p in planilhas:
            if p["name"] not in selecionadas:
                continue  # pula as não selecionadas

            sid = p["id"]
            p_name = p["name"]

            # Se já auditada, pula ou atualiza? Aqui atualiza para garantir dados frescos
            # ... seu código de auditoria para essa planilha ...

            # Salvar resultado parcial no session_state
            st.session_state.resultados_audit[sid] = {
                "Planilha": p_name,
                "Origem": v_o,
                "DRE": v_d,
                "MP DRE": v_mp_d,
        ...
        # Mostrar tabela com todos os resultados já auditados (parciais ou completos)
        resultados = [v for v in st.session_state.resultados_audit.values() if v is not None]
        if resultados:
