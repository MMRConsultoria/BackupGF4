...
# ---------- BLOQUEIO DE ACESSO (EM-CÓDIGO) ----------
# Conjuntos de e-mails autorizados
...
USUARIOS_ADM = {
    "seu.admin@exemplo.com",
    # adicione admins aqui (tem acesso às abas restritas)
}

# Helpers
def _try_detect_streamlit_user():
    """Tenta obter email do objeto experimental_user (Streamlit Cloud/Enterprise)."""
    try:
        user = getattr(st, "experimental_user", None)
        if user:
            # em alguns ambientes user pode ser dict-like ou objeto com 'email'
...
def _set_session_user(email, role="user"):
    st.session_state["usuario_logado"] = email
    st.session_state["role"] = role

# 1) primeiro: usar valor já setado em session_state (se houver)
usuario_logado = st.session_state.get("usuario_logado")

# 2) tentar detectar do Streamlit/SSO automaticamente
if not usuario_logado:
    detected = _try_detect_streamlit_user()
    if detected:
        usuario_logado = detected
        # atribuir role automaticamente se for admin
        role = "admin" if detected in USUARIOS_ADM else "user"
        _set_session_user(detected, role)

# 3) se ainda não detectado, mostrar um pequeno form de login por e-mail (fallback)
if not usuario_logado:
    st.markdown("### Área restrita — identifique-se")
    col_a, col_b = st.columns([4,1])
    with col_a:
        email_input = st.text_input("Seu e-mail (somente para identificação)", value="", placeholder="email@dominio.com", key="login_email")
    with col_b:
        entrar = st.button("Entrar", key="login_entrar")
    if entrar:
        if not email_input:
            st.error("Informe um e-mail.")
        else:
            email_norm = email_input.strip().lower()
            # aqui não há senha — adequado apenas para uso interno/dev
            role = "admin" if email_norm in USUARIOS_ADM else "user"
            _set_session_user(email_norm, role)
            st.experimental_rerun()
    # se não clicou em Entrar, interrompe a execução do resto do app
    st.stop()

# A partir daqui temos um usuario_logado definido (via session_state)
usuario_logado = st.session_state.get("usuario_logado")
current_role = st.session_state.get("role", "user")

# Verificar autorização (RH)
if usuario_logado not in USUARIOS_AUTORIZADOS_RH and usuario_logado not in USUARIOS_ADM:
    st.warning("⛔ Acesso restrito — usuário não autorizado.")
    st.stop()

# Mostra quem está logado e botão de logout
col_info, col_out = st.columns([9,1])
with col_info:
    st.markdown(f"**Conectado como:** `{usuario_logado}`  — role: `{current_role}`")
with col_out:
    if st.button("Logout", key="btn_logout"):
        for k in ("usuario_logado", "role"):
