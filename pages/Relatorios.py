...
with st.spinner("⏳ Processando..."):
    ...
    with aba5:
        ...
        try:
            ...
            # ⚠️ não faça conversão de moeda aqui para evitar conversão dupla
            # (deixamos tudo para a sub-aba SANGRIA com parse_brl_str)
            ...
            if df_sangria is None or df_sangria.empty:
                ...
                if visao == "Analítico":
                    ...
                    if not col_valor or "Loja" not in df_fil.columns or "Descrição Agrupada" not in df_fil.columns:
                        ...
                        # soma por Loja + Descrição Agrupada
                        df_agg = (
                    ...
                    if "Data" not in base.columns or "Código Everest" not in base.columns or not col_valor:
                        ...
                        if not all([col_emp, col_dt, col_val]):
                            ...
                            df_ev["Valor Lancamento"] = pd.Series(df_ev[col_val]).apply(parse_brl_str).astype(float)
                            df_ev = df_ev[(df_ev["Data"].dt.date >= dt_inicio) & (df_ev["Data"].dt.date <= dt_fim)]
                            df_ev["Sangria Everest"] = df_ev["Valor Lancamento"].abs()
                            df_ev_agg = (df_ev.groupby(["Código Everest","Data"], as_index=False)["Sangria Everest"].sum())
                
                            # Merge e diferença
                            df_cmp = df_sys.merge(df_ev_agg, on=["Código Everest","Data"], how="outer").fillna(0.0)
                            df_cmp["Diferença"] = df_cmp["Sangria (Sistema)"] - df_cmp["Sangria Everest"]
                
                            # Junta Loja/Grupo (Tabela Empresa), se existir
                            if "Código Everest" in df_empresa.columns:
                                mapa = df_empresa[["Código Everest","Loja","Grupo"]].drop_duplicates().copy()
                                mapa["Código Everest"] = mapa["Código Everest"].astype(str).str.extract(r"(\d+)")
                                df_cmp = df_cmp.merge(mapa, on="Código Everest", how="left")
                
                            # Se filtro "Diferenças Everest", mostra só linhas com diferença != 0
                            import numpy as np
                            if visao == "Diferenças Everest":
                                df_cmp = df_cmp[np.isclose(df_cmp["Diferença"], 0.0) == False]
                
                            # Ordena, totaliza e formata
                            df_cmp = df_cmp[["Grupo","Loja","Código Everest","Data",
                                             "Sangria (Sistema)","Sangria Everest","Diferença"]].sort_values(
        ...
        with sub_evx:
            if df_sangria is None or df_sangria.empty:
                ...
                # re-detecta coluna de valor (não depende da sub-aba anterior)
