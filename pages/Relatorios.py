            ...
            if df_sangria is None or df_sangria.empty:
                ...
                if visao == "Analítico":
                    ...
                    if not df_exibe.empty:
                        _render_df(df_exibe.drop(columns=["Data"], errors="ignore").assign(Data=df_exibe["Data"]), height=480)
                
                        buf = BytesIO()
                        with pd.ExcelWriter(buf, engine="openpyxl") as w:
                            df_exibe.to_excel(w, index=False, sheet_name="Comparativa_Everest")
                        buf.seek(0)
                        st.download_button("⬇️ Baixar Excel (Comparativa Everest)", buf, "comparativa_everest.xlsx")
