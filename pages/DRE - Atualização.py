...
def get_conf_map(sheet_ids, target_name):
    ...
    for sid in sheet_ids:
        try:
            ...
            # Obtém todos os títulos das abas e limpa espaços
...
if s_ids:
    with st.spinner("Buscando planilhas e verificando abas..."):
        ...
        if planilhas:
            ...
            # Verificação da aba Configurações
            ...
            with col_t1:
                st.data_editor(df.iloc[:meio], key="t1", use_container_width=True, column_config=config, hide_index=True)
            with col_t2:
                st.data_editor(df.iloc[meio:], key="t2", use_container_width=True, column_config=config, hide_index=True)
            
            st.button("🚀 INICIAR ATUALIZAÇÃO", use_container_width=True)
