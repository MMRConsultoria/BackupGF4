    ...
    if df_sangria is None or df_sangria.empty:
        ...
        if visao == "Comparativa Everest":
            ...
            if "Data" not in base.columns or "Código Everest" not in base.columns or not col_valor:
                ...
                # Colunas ocultas na visão (mas preservadas no raw p/ update por Duplicidade)
                _cols_hide = ["Mês", "Mes", "Ano", "Duplicidade", "Sistema"]
        
                # ====================== estado de seleção por CÓDIGO ======================
                def _only_digits(x):
                ...
                # ====================== helpers ======================
                WS_SISTEMA = "Sangria"  # <— nome da aba do sistema no Google Sheets (onde tem Duplicidade e Descrição Agrupada)
                ...
                def _find_col(df: pd.DataFrame, target_name: str):
                ...
                def _sheet_df_with_row(ws):
                    ...
                    col_map = {c: i+1 for i, c in enumerate(header)}  # nome exato -> índice 1-based
                    return df_ws, col_map
        
                def _desc_options_flex(fallback_df: pd.DataFrame) -> list[str]:
                    try:
                        col_desc_tbl = _find_col(df_descricoes, "Descrição Agrupada")
                        base_opts = set(df_descricoes[col_desc_tbl].astype(str).dropna().unique())
                    except Exception:
                        base_opts = set()
                    cur_opts = set()
                    dc = _find_col(fallback_df, "Descrição Agrupada")
                    if dc:
                        cur_opts = set(fallback_df[dc].astype(str).dropna().unique())
                    opts = sorted({o.strip() for o in (base_opts | cur_opts) if o and o.strip()})
                    return opts if opts else ["Outros"]
        
                def _ws_desc_col_idx(df_ws: pd.DataFrame, col_map: dict) -> int | None:
                    for cname in df_ws.columns:
                        if _norm_name(cname) == _norm_name("Descrição Agrupada"):
                            return col_map.get(cname)
                    return None
        
                # ======================================================================
                # EXPANDER: INCLUÍDOS (pode editar 'Descrição Agrupada' e gravar no Sheets por Duplicidade)
                # ======================================================================
                with st.expander("🧾 Ver itens incluídos (Colibri/CISS)"):
                    audit_in_raw = base.loc[~mask_dep_sys, :].copy()  # mantém Duplicidade para chave
                    ...
                    desc_col_view_in = _find_col(audit_in_view, "Descrição Agrupada")
                    col_cfg_in = {c: cc.TextColumn(disabled=True, label=c) for c in audit_in_view.columns}
        
                    have_dropdown_in = hasattr(cc, "SelectboxColumn") and bool(desc_col_view_in)
                    if have_dropdown_in:
                        col_cfg_in[desc_col_view_in] = cc.SelectboxColumn(
                    ...
                    with st.form("form_editar_desc_incluidos", clear_on_submit=False):
                        ...
                        # Fallback se não houver SelectboxColumn
                        changes_in = {}
                        if not have_dropdown_in:
                            st.info("Seu runtime do Streamlit não oferece dropdown na grade; use o editor auxiliar abaixo.")
                            dup_col_in_raw = _find_col(audit_in_raw, "Duplicidade")
                            vis_keys = audit_in_raw[dup_col_in_raw].astype(str).tolist() if dup_col_in_raw else []
                            sel_dups = st.multiselect(
                                "Escolha as duplicidades a alterar:",
                                options=vis_keys,
                                max_selections=1000,
                            )
                            nova_desc_in = st.selectbox("Nova 'Descrição Agrupada':", options=opcoes_desc_in, index=0)
                            if sel_dups and nova_desc_in:
                                for k in sel_dups:
                                    changes_in[k] = nova_desc_in
                    ...
                    if salvar_in:
                        try:
                            ...
                            if df_ws.empty:
                                st.error("A aba do Sheets está vazia.")
                            else:
                                col_idx_desc = _ws_desc_col_idx(df_ws, col_map)
                                dup_ws_col = _find_col(df_ws, "Duplicidade")
                                if (col_idx_desc is None) or (dup_ws_col is None):
                                    st.error("A aba do Sheets precisa ter as colunas 'Duplicidade' e 'Descrição Agrupada'.")
                                else:
                                    updates = []
                                    if have_dropdown_in:
                                        desc_col_raw_in = _find_col(audit_in_raw, "Descrição Agrupada")
                                        antes  = audit_in_raw[desc_col_raw_in].astype(str).fillna("").reset_index(drop=True)
                                        depois = edited_in_view[desc_col_view_in].astype(str).fillna("").reset_index(drop=True)
                                        mask_changed = (antes != depois)
                                        keys = audit_in_raw[_find_col(audit_in_raw, "Duplicidade")].astype(str).reset_index(drop=True)
                                        for i in mask_changed[mask_changed].index:
                                            dup_key   = keys.iloc[i]
                                            nova_desc = depois.iloc[i].strip()
                                            hits = df_ws.index[df_ws[dup_ws_col] == dup_key].tolist()
                                            for h in hits:
                                                updates.append(Cell(row=int(df_ws.loc[h, "_row"]), col=col_idx_desc, value=nova_desc))
                                    else:
                                        for dup_key, nova_desc in changes_in.items():
                                            hits = df_ws.index[df_ws[dup_ws_col] == str(dup_key)].tolist()
                                            for h in hits:
                                                updates.append(Cell(row=int(df_ws.loc[h, "_row"]), col=col_idx_desc, value=nova_desc))
                ...
                # ======================================================================
                # EXPANDER: REMOVIDOS (pode editar 'Descrição Agrupada' e gravar no Sheets por Duplicidade)
                # ======================================================================
                with st.expander("🔎 Ver depósitos removidos (Colibri/CISS)"):
                    ...
                    desc_col_view_out = _find_col(audit_out_view, "Descrição Agrupada")
                    col_cfg_out = {c: cc.TextColumn(disabled=True, label=c) for c in audit_out_view.columns}
        
                    have_dropdown_out = hasattr(cc, "SelectboxColumn") and bool(desc_col_view_out)
                    if have_dropdown_out:
                        col_cfg_out[desc_col_view_out] = cc.SelectboxColumn(
                            label="Descrição Agrupada",
                    ...
                    with st.form("form_editar_desc_removidos", clear_on_submit=False):
                        ...
                        changes_out = {}
                        if not have_dropdown_out:
                            st.info("Seu runtime do Streamlit não oferece dropdown na grade; use o editor auxiliar abaixo.")
                            dup_col_out_raw = _find_col(audit_out_raw, "Duplicidade")
                            vis_keys = audit_out_raw[dup_col_out_raw].astype(str).tolist() if dup_col_out_raw else []
                            sel_dups_out = st.multiselect(
                                "Escolha as duplicidades a alterar (removidos):",
                                options=vis_keys,
                                max_selections=1000,
                            )
                            nova_desc_out = st.selectbox("Nova 'Descrição Agrupada':", options=opcoes_desc_out, index=0, key="sb_rem")
                            if sel_dups_out and nova_desc_out:
                                for k in sel_dups_out:
                                    changes_out[k] = nova_desc_out
                        ...
                        try:
                            ...
                            if df_ws.empty:
                                ...
                                if (col_idx_desc is None) or (dup_ws_col is None):
                                    ...
                                    updates = []
                                    if have_dropdown_out:
                                        desc_col_raw_out = _find_col(audit_out_raw, "Descrição Agrupada")
                                        antes  = audit_out_raw[desc_col_raw_out].astype(str).fillna("").reset_index(drop=True)
                                        depois = edited_out_view[desc_col_view_out].astype(str).fillna("").reset_index(drop=True)
                                        mask_changed = (antes != depois)
                                        keys = audit_out_raw[_find_col(audit_out_raw, "Duplicidade")].astype(str).reset_index(drop=True)
                                        for i in mask_changed[mask_changed].index:
                                            dup_key   = keys.iloc[i]
                                            nova_desc = depois.iloc[i].strip()
                                            hits = df_ws.index[df_ws[dup_ws_col] == dup_key].tolist()
                                        ...
                                        for dup_key, nova_desc in changes_out.items():
                                            hits = df_ws.index[df_ws[dup_ws_col] == str(dup_key)].tolist()
                                            for h in hits:
                                                updates.append(Cell(row=int(df_ws.loc[h, "_row"]), col=col_idx_desc, value=nova_desc))
                ...
                # ======================================================================
                # Fluxo normal (comparativo) — usa apenas INCLUÍDOS
                # ======================================================================
                ...
                if not all([col_emp, col_dt_ev, col_val_ev]):
                    ...
                    # ======================================================================
                    # Botões Selecionar/Limpar (após os depósitos) + Tabela com checkbox
                    # ======================================================================
