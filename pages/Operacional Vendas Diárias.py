...
def _aplicar_atualizacoes_google_sheets(
    ...
    def enviar_para_sheets(df_input: pd.DataFrame, titulo_origem: str = "dados") -> bool:
        ...
        with st.spinner(f"🔄 Processando {titulo_origem} e verificando duplicidades..."):
            ...
            if len(df_suspeitos) > 0:
                st.warning("⚠️ Existem suspeitos (chave N). Resolva no envio automático antes de salvar manuais.")
                return False
    ...
    # ---------- ENVIO AUTOMÁTICO (com resolução de conflitos) ----------
    if enviar_auto:
        if 'df_final' not in st.session_state or st.session_state.df_final.empty:
            ...
            with st.spinner("🔄 Processando dados e verificando duplicidades..."):
                ...
                df_base['Data'] = pd.to_datetime(
                    df_base['Data'].astype(str).replace("'", "", regex=True).str.strip(),
                    dayfirst=True, errors="coerce"
                )
                df_base['Data'] = (df_base['Data'] - pd.Timestamp("1899-12-30")).dt.days

                for coln in ['Fat.Total','Serv/Tx','Fat.Real','Ticket']:
                    if coln in df_base.columns:
                        df_base[coln] = pd.to_numeric(df_base[coln], errors="coerce").fillna(0.0)
                ...
                if 'Ano' in df_base.columns:
                    df_base['Ano'] = df_base['Ano'].apply(to_int_safe)
                if 'Código Everest' in df_base.columns:
                    df_base['Código Everest'] = df_base['Código Everest'].apply(to_int_safe)
                if 'Código Grupo Everest' in df_base.columns:
                    df_base['Código Grupo Everest'] = df_base['Código Grupo Everest'].apply(to_int_safe)
                ...
                fat_col  = pick_col(df_base, headers, "fat total", "Fat.Total", "Fat Total")
                loja_col = pick_col(df_base, headers, "loja", "Loja")
                data_col = pick_col(df_base, headers, "data", "Data")
                cod_col  = pick_col(df_base, headers, "codigo everest", "Codigo Everest", "Código Everest")
                ...
                # Data_Formatada / M / N
                df_base["Data_Formatada"] = pd.to_datetime(
                    df_base[data_col], origin="1899-12-30", unit="D", errors="coerce"
                ).dt.strftime("%Y-%m-%d")

                df_base[fat_col] = pd.to_numeric(df_base[fat_col], errors="coerce").fillna(0)
                df_base[cod_col] = pd.to_numeric(df_base[cod_col], errors="coerce").fillna(0).astype(int).astype(str)

                df_base["M"] = (df_base["Data_Formatada"].fillna("") + df_base[fat_col].astype(str) + df_base[loja_col].astype(str)).str.strip()
                df_base["N"] = (df_base["Data_Formatada"].fillna("") + df_base[cod_col].astype(str)).str.strip()
                df_base = df_base.drop(columns=["Data_Formatada"], errors="ignore")

                # reindex conforme o sheet
                extras = [c for c in df_base.columns if c not in headers]
                df_final = df_base.reindex(columns=headers + extras, fill_value="")
                ...
                if len(df_novos) > 0 and "Data" in df_novos.columns:
                    df_tmp = df_novos.copy()
                    df_tmp["Data"] = pd.to_datetime(df_tmp["Data"], origin="1899-12-30", unit="D", errors="coerce").dt.strftime("%d/%m/%Y")
                    st.dataframe(df_tmp, use_container_width=True)
                elif len(df_novos) == 0:
                    st.info("Nenhum novo registro encontrado.")

                # Se NÃO houver suspeitos -> envia direto os novos e formata
                if len(df_suspeitos) == 0:
                    if len(df_novos) == 0:
                        ...
                        if inicio <= fim:
                            ...
                            format_cell_range(aba_destino, f"F{inicio}:F{fim}", numero_format)
                            format_cell_range(aba_destino, f"L{inicio}:L{fim}", numero_format)
                        st.success(f"✅ {len(dados_para_enviar)} novo(s) enviados.")
                    st.stop()
